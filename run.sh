...
SUPPORTED_TARGETS="nginx-proxy"

EXEC_CMD=""
EXEC_ITEMS_LIST=""

DEPLOY_ROOT=${HOME}/servers
NGINX_PROXY_DIR=nginx-proxy
NGINX_PROXY_HOME=${DEPLOY_ROOT}/${NGINX_PROXY_DIR}
...
deploy_nginx-proxy()
{
    ...
    then
        ...
        exit 1
    elif [ -d ${NGINX_PROXY_HOME} ]
    then
        echoY "Already deployed nginx-proxy in ${NGINX_PROXY_HOME}, check it there first!"
        exit 1
    else
        echoY "Deploying nginx-proxy..."
        cp -a ./src ${NGINX_PROXY_HOME}
        echoG "nginx-proxy has been deployed to ${NGINX_PROXY_HOME} successfully."
        cat ./src/README.md
    fi
}
